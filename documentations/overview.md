...
1. **Fetches match history** from an upstream legacy API (one-time backfill periodically)
2. **Listens for live matches** via Server-Sent Events (SSE stream)
3. **Stores everything** in PostgreSQL (players, matches, sync state)
4. **Serves HTTP endpoints** to query matches and leaderboards
5. **Broadcasts real-time updates** to connected clients via SSE
